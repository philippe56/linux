...
int ocxl_afu_map_lpc_mem(struct ocxl_afu *afu)
{
	...
	if ((afu->config.lpc_mem_size == 0) && (afu->config.special_purpose_mem_size == 0))
		return 0;
	...
	if (afu->config.special_purpose_mem_size) {
		afu->special_purpose_res.start = afu->lpc_base_addr + afu->config.special_purpose_mem_offset;
		afu->special_purpose_res.end = afu->special_purpose_res.start + afu->config.special_purpose_mem_size - 1;
	}
